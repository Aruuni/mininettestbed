...
def run_simulation(*args):
    ...
    path = f"{HOME_DIR}/cctestbed/ns3/cross_path_fairness_inter_rtt_50mx_fixed/{aqm}/{topology}_{bw}mbit_{delay}ms_{int(qsize_in_bytes_2/1500)}pkts_{loss}loss_{n_flows}flows_{tcp_buffer_mult}tcpbuf_{protocol}/run{run}/" 
    ...
    command = f'cd {HOME_DIR}/ns-3-dev; time ./ns3 run --no-build "scratch/cross_path.cc --configJSON={path}emulation_info.json --path={path} --numClients={2} --delay1={25} --delay2={delay} --bandwidth1={bw} --bandwidth2={bw} --queuesize1={int(qsize_in_bytes_1/1500)} --queuesize2={int(qsize_in_bytes_2/1500)} --seed={run}" > {path}output.txt 2>&1'
    printGreen(command)
    printRed(path)
