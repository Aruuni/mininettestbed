...
def run_simulation(*args):
    ...
    command = f'cd {HOME_DIR}/ns-3-dev; time ./ns3 run --no-build "scratch/CCTestBed.cc --configJSON={path}/emulation_info.json --path={path} --delay={5} --delay2={delay}--bandwidth={bw} --queuesize={int(qsize_in_bytes/1500)} --seed={run}" > {path}/output.txt 2>&1'
    printGreen(command)
