...
def run_emulation(topology: str, protocol: str, params, bw: int, delay:int, qmult:float, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2, bw2=None) -> None:
    ...
    traffic_config.append(TrafficConf(f"c2{params['ex_idx']}", f"x2{params['ex_idx']}", 30, DURATION-30, "cubic"))

    flows_order = [0, 30_000]
    ...
    em = Emulation(net, network_config, traffic_config, path, 0.1, False, params['ex_idx'], {"env_bw": bw, "scheme": protocol, "bw2": bw, "bw2_flip_period": 1000, "trace_file": trace_mame, "flows_order": flows_order, "timestamp": timestamp, 'actor_version': "sage", 'trace_set':"sage_traces2" }) 
