...
def run_emulation(topology, protocol, params, bw, delay, qmult, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2):
    ...
    path = "%s/cctestbed/mininet/results_friendly_intra_rtt_async_inverse_longer/%s/%s_%smbit_%sms_%spkts_%sloss_%sflows_%stcpbuf_%s/run%s" % (HOME_DIR, aqm, topology, bw, delay, int(qsize_in_bytes/1500), loss, n_flows, tcp_buffer_mult, protocol, run)
    rmdirp(path)
    mkdirp(path)
    if (protocol == "bbr3"):
        protocol = "bbr"
    ...
    if n_flows == 1:
        ...
        traffic_config = [TrafficConf('c1', 'x1', 0, 600, 'cubic'),
                           TrafficConf('c2', 'x2', int(duration/2), 600-int(duration/2), protocol)]
