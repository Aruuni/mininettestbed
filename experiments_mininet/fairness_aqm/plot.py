...
def plot_run(*args):
    ...
    path = "%s/cctestbed/mininet/results_fairness_aqm/%s/%s_%smbit_%sms_%spkts_%sloss_%sflows_%stcpbuf_%s/run%s" % (HOME_DIR,aqm, topology, bw, delay, int(qsize_in_bytes/1500), loss, n_flows, tcp_buffer_mult, protocol, run)
...
if __name__ == '__main__':
    ...
    PROTOCOLS = ['bbr1', 'cubic',  'astraea', 'bbr3', 'orca', 'sage', 'vivace-uspace']
    ...
    params_list = [("Dumbell", protocol, {'n':2}, bw, delay, mult, 22, run, "fifo", 0, 4)
