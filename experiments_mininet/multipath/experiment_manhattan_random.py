...
def run_emulation(topology, protocol, params, bw, delay, qmult, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2, n_subflows=2, controller_name='None', path_selector="None", num_paths=8, path_penalty=100, seed="NO-SEED", path_selector_preset="None",  output_folder="mininet"):
    ...
    Start up hosts at unique satellite positions, flows start 5 seconds apart and end at the same time.
    ...
    output_path = f"{HOME_DIR}/cctestbed/{output_folder}/results_manhattan_openflow_random/{topology}_{seed}_{n_flows}flows/{aqm}_{bw}mbit_{delay}ms_{int(qsize_in_bytes/1500)}pkts_{loss}loss_{tcp_buffer_mult}tcpbuf/{controller_path}/{protocol}_{n_subflows}subflows/run{run}" 
    ...
    for h, host in enumerate(net.hosts):
        ...
        net.addLink(f'UT_{host.name}', f's{x}_{y}') # issue is probably here. Things work if I do it at the topo class. #self.addLink(ut, f's{x}_{y}')
