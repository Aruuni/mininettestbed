...
def run_emulation(topology, protocol, params, bw, delay, qmult, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2, n_subflows=2, controller_name='None', path_selector="None", num_paths=8, path_penalty=100, seed="NO-SEED", path_selector_preset="None",  output_folder="mininet"):
    ...
    for f in range(1, n_flows + 1):
        network_config.append(NetworkConf(f's_c{f}', f'UT_c{f}', None,   host_delay,    3*bdp_in_bytes, True,  'fifo',  loss)) # Client<->Sat delay (Reported by starlink to be 1.8-3.6ms one-way)
        network_config.append(NetworkConf(f's_x{f}', f'UT_x{f}', None,   host_delay,    3*bdp_in_bytes, True,  'fifo',  loss)) # Server<->Sat delay (Reported by starlink to be 1.8-3.6ms one-way)
