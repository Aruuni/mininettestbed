...
def run_emulation(topology: str, protocol, params, bw, delay, qmult, tcp_buffer_mult=3, run=0, aqm='fifo', loss=None, n_flows=2):
    ...
    path = f"{HOME_DIR}/cctestbed/mininet/results_parking_lot_inter_rtt/{aqm}/{topology}_{bw}mbit_{delay}ms_{int(qsize_in_bytes/1500)}pkts_{loss}loss_{n_flows}flows_{tcp_buffer_mult}tcpbuf_{protocol}/run{run}" 
    ...
    delay_config = [NetworkConf(f'c{i}', f's{i-1}', None, 2*10, 3*qsize_in_bytes, False, aqm, loss) for i in range(2, n_flows+1,1)]
    delay_config.append(NetworkConf('c1', 's1', None, 2*delay*3, 3*qsize_in_bytes, False, aqm, loss))
    ...
    printBlue(network_config)
    printGreen(delay_config)
