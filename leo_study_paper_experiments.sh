...
do
    ...
    do
        ...
        do
            for run in $RUNS
            do
                run experiments_mininet/intra_bw_fairness.py "20" $bw $qmult $protocol $run fifo 0 "2"
            done
        done  
    done
done
...
do
    ...
    do
        ...
        do
            for run in $RUNS
            do
                run experiments_mininet/cubic_rtt_fairness.py $del "100" $qmult $protocol $run fifo 0 "2"
            done
        done
...
do
    ...
    do
        for run in $RUNS
        do
            run experiments_mininet/cubic_rtt_fairness_inverse.py $DELAYPOINT "100" $qmult $protocol $run fifo 0 "2"
        done
    done
done
    ...
    do
        ...
        do
            ...
            do
                for run in $RUNS
                do
                    run experiments_mininet/fairness_aqm.py $del "100" $qmult $protocol $run $aqm 0 "4"
                done
            done
        done
    done
...
do
    ...
    do
        ...
        do
            for run in $RUNS
            do
                run experiments_mininet/parking_lot.py $del "100" $qmult $protocol $run "fifo" 0 "4"
            done
        done
    done
done
