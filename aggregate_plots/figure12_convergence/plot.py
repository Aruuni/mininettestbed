...
def plot_data(data, filename, ylim=None, xlim=None):
    LINEWIDTH = 1
    fig, axes = plt.subplots(nrows=len(PROTOCOLS_EXTENSION), ncols=1, figsize=(8, 5), sharex=True, sharey=True)
    for i, protocol in enumerate(PROTOCOLS_EXTENSION):
        ...
        ax.text(70, 85, PROTOCOLS_FRIENDLY_NAMES[protocol], va='center', c=COLORS_EXTENSION[protocol])
        ax.grid()

    fig.text(0.045, 0.5, 'Goodput (Mbps)', va='center', rotation='vertical', fontsize=16)
