...
EXPERIMENT_PATH = f"{HOME_DIR}/cctestbed/gauntlet/results_fairness_inter_rtt/fifo" 
    ...
    for protocol in PROTOCOLS_EXTENSION:
        for bw in BWS:
            for delay in DELAYS:
                ...
                for run in RUNS:
                    PATH = f"{EXPERIMENT_PATH}/Dumbell_{bw}mbit_{delay}ms_{int(mult * BDP_IN_PKTS)}pkts_{protocol}/run{run}/csvs" 
