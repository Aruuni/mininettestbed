...
for flow in FLOWS:
   ...
   summary_data = pd.DataFrame(data,
                              columns=['protocol', 'bandwidth', 'delay', 'delay_ratio','flows', 'goodput_ratio_total_mean', 'goodput_ratio_total_std'])
   ...
   for format in ['pdf']:
      plt.savefig('goodput_ratio_between_hops_goodput_flows%s.%s' % (flow, format), dpi=1080)
