...
def get_rtt_df(ROOT_PATH, PROTOCOLS, RUNS, BW, DELAY, QMULT):
    ...
    for protocol in PROTOCOLS:
        for run in RUNS:
            PATH = ROOT_PATH + f'/Dumbell_{BW}mbit_{DELAY}ms_{int(QMULT * BDP_IN_PKTS)}pkts_0loss_1flows_22tcpbuf_{protocol}/run{run}'
            with open(PATH + '/emulation_info.json', 'r') as fin:
                emulation_info = json.load(fin)
...
# Load RTT Data
rtt_data_bw_rtt = get_rtt_df(f"{HOME_DIR}/cctestbed/mininet/results_responsiveness_bw_rtt_leo/fifo", PROTOCOLS, RUNS, BW, DELAY, QMULT)
rtt_data_loss = get_rtt_df(f"{HOME_DIR}/cctestbed/mininet/results_responsiveness_bw_rtt_loss_leo/fifo", PROTOCOLS, RUNS, BW, DELAY, QMULT)
...
for protocol in PROTOCOLS:
    ...
    rtts = rtt_data_loss[rtt_data_loss['protocol'] == protocol]['srtt']
    values, base = np.histogram(rtts, bins=BINS)
    cumulative = np.cumsum(values)
    ax.plot(base[:-1], cumulative / 50 * 100, label=f"{protocol}-loss", linestyle='dashed', c=COLOR[protocol])

ax.set(xlabel="RTT (ms)", ylabel="Percentage of Trials (\%)")
ax.annotate('optimal', xy=(50, 50), xytext=(35, 80), arrowprops=dict(arrowstyle="->", linewidth=0.5))
