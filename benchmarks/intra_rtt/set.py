...
mpl.rcParams.update({
    "figure.figsize": (12.5, 5.2),   # wider to fit two side-by-side
    "axes.titlesize": 12,
    "axes.labelsize": 11,
    "xtick.labelsize": 10,
    "ytick.labelsize": 10,
...
def load_pair_timeseries(PATH: str, protocol: str, delay_ms: int) -> Tuple[Optional[pd.Series], Optional[pd.Series]]:
    ...
    # per your note: use srtt / delay (no *2)
...
# ---------- NEW: axis-based plotting helpers (no file saving) ----------
def _annotate_cells(ax, im, matrix):
...
def plot_heatmap_ratio_sequential_ax(ax, matrix, bws, delays, title, cbar_label):
    ...
    _annotate_cells(ax, im, matrix)
    # return image for colorbar creation by caller
    return im

def plot_delay_ratio_one_sided_ax(ax, matrix, bws, delays, title, cbar_label, vmin=1.0, vmax=None):
...
# ---------- (Original file-saving helpers kept; safe to remove if unused) ----------
def plot_heatmap_ratio_sequential(matrix, bws, delays, title, cbar_label, outfile):
    fig, ax = plt.subplots()
    im = plot_heatmap_ratio_sequential_ax(ax, matrix, bws, delays, title, cbar_label)
...
def plot_delay_ratio_one_sided(matrix, bws, delays, title, cbar_label, outfile, vmin=1.0, vmax=None):
    fig, ax = plt.subplots()
    im = plot_delay_ratio_one_sided_ax(ax, matrix, bws, delays, title, cbar_label, vmin=vmin, vmax=vmax)
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label(cbar_label)
    fig.tight_layout()
    plt.savefig(outfile, dpi=400, bbox_inches='tight')
    ...
    with PdfPages(out_pdf) as pdf:
        for aqm in AQMS:
            for protocol in PROTOCOLS:
                ...
                # Build one page (figure) with two side-by-side heatmaps
                fig, axes = plt.subplots(1, 2, figsize=mpl.rcParams["figure.figsize"])
                ax1, ax2 = axes

                # Left: Goodput ratio
                im1 = plot_heatmap_ratio_sequential_ax(
                    ax1,
                    goodput_ratio_mat,
                    BWS, DELAYS,
                    title=f"Goodput Ratio min/max at each timestep (avg over window)\n{friendly} / {aqm.upper()}",
                    cbar_label="Goodput Ratio"
                )
                cbar1 = fig.colorbar(im1, ax=ax1)
                cbar1.set_label("Goodput Ratio")
                cbar1.formatter = ScalarFormatter(); cbar1.update_ticks()

                # Right: Delay ratio
                im2 = plot_delay_ratio_one_sided_ax(
                    ax2,
                    delay_ratio_mat,
                    BWS, DELAYS,
                ...
                cbar2.set_label("avg srtt / (delay)")

                # A tidy super-title for the page
                fig.suptitle(f"{friendly} – {aqm.upper()}", y=1.03, fontsize=13)
                fig.tight_layout()

                # Commit the page to the PDF
