...
def plot_one(QMULT, RUN):
   ...
   PROTOCOL_DATA = {'cubic': {'x1': None, 'y1': None,'x2': None, 'y2': None},
                    ...
                    'bbr1': {'x1': None, 'y1': None,'x2': None, 'y2': None}}
   # Get the data:
   ...
   max_bbr1_y = max(PROTOCOL_DATA['bbr1']['y1'].max(), PROTOCOL_DATA['bbr1']['y2'].max())
   max_y = max(max_cubic_y, max_bbr_y, max_bbr1_y)
...
if __name__ == "__main__":
   for mult,run in zip([0.2,1,4],[1,1,1]):
      plot_one(mult,run)
